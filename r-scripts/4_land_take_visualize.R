...
labelX <- 'natural/semi-natural land'
labelY <- 'cropland'
title <- 'a'
...
g1 <- legend_3 %>%
  ...
  labs(x = paste0(labelX, '  →'), y = paste0(labelY, '  →'),
       title = expression("Land take"~km^2)) +
...
ltNatreGrid <- st_read('./data/land_take_inside_grid_top_nature_select.kml') %>% #bb63b1 
  st_transform(st_crs(3035)) 
ltCroplandGrid <- st_read('./data/land_take_inside_grid_top_cropland_select.kml') %>% #63c8c9 
  st_transform(st_crs(3035))
...
ggplot() + geom_sf(data = grid_sf) + geom_sf(data = ltNatreGrid %>%
                                               st_bbox()%>%
                                               st_as_sfc() , color='red')
...
m1 <- ggplot() +
  ...
  geom_sf(data = ltNatreGrid %>%
            st_bbox()%>%
            st_as_sfc(), fill=NA, color='red', linewidth=1) +
  scale_fill_identity() + 
