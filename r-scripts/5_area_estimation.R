...
mappedSamples <- read_csv('./data/For_GEE/samples_stratified_final.csv') %>%
  distinct(PLOTID, country, stratum)
unique(mappedSamples$country)
length(unique(mappedSamples$PLOTID))
nrow(mappedSamples)
...
# Write out for later
referenceSamples %>%
  write_csv('./data/reference_samples_clean_final.csv')
...
mergedSamples <- mappedSamples %>% as_tibble() %>% 
  ...
  drop_na(r)
nrow(mergedSamples)
length(unique(mergedSamples$PLOTID))
...
for (c in unique(mappedSamples$country)){
  ...
  for (takeType in c('merged', 'separate')){
    ...
    r <- as.character(unlist(combinedForEstimation$r))
    m <- as.character(unlist(combinedForEstimation$m))
    e<-olofsson(r,m, Nh)
...
sum(area_mapped_countries$area)
...
lueLandTakePlot <- lueDFlandtake %>%
  ...
  labs(x = '',
       title = 'a',
       y = expression('Land take total ('~m^-2~citizen^-1~yr^-1~')')) +
...
luefNaturePlot <- lueDFnature %>%
  ...
  labs(x = '',
       title = 'b',
       y = expression('Land take natural/semi-natural ('~m^-2~citizen^-1~yr^-1~')'))+
  theme(axis.title.x = element_text(size=9),
        axis.title.y = element_blank())
luefNaturePlot
...
luefCroplandPlot <- lueDFcropland %>%
  ...
  labs(x = '',
       title = 'c',
       y = expression('Land take cropland ('~m^-2~citizen^-1~yr^-1~')'))+
  theme(axis.title.x = element_text(size=9),
        axis.title.y = element_blank())
luefCroplandPlot
...
# Calculate the Marginal Land Consumption rate
# land take divide pop change
areaEstTotCountry <- read_csv('./data/area_estimates_countries.csv') %>%
  mutate(unbiased_area = area)

population2018 <- pop %>%
  filter(Time == 2018) %>%
  dplyr::select(country, pop2018=PopTotal)
popChange <- population2023 %>%
  left_join(population2018) %>%
  mutate(popChg = PopTotal - pop2018) %>%
  dplyr::select(country, popChg)


lueChgDFlandtake <- areaEstCountry %>%
  filter(takeType == 'merged') %>%
  filter(stratum %in% c('land take')) %>%
  dplyr::select(country, areaEst = unbiased_area, ciArea) %>%
  left_join(popChange, by = 'country') %>%
  mutate(lue = (areaEst*1000000) / (popChg*1000),
         lueCI = (ciArea*1000000) / (popChg*1000)) 

lueChgLandTakePlot <- lueChgDFlandtake %>%
  ggplot(aes(x = reorder(country, lue), y = lue)) +
  ...
  labs(x = '',
       title = 'a',
       y =expression(atop('Land take total', '('~m^-2~Delta~citizen^-1~')')))+
  theme(axis.title.x = element_text(size=9),
        axis.title.y = element_blank())
lueChgLandTakePlot


lueChgDFnature <- areaEstCountry%>%
  filter(takeType == 'separate')%>%
  filter(stratum == 'nature loss') %>%
  dplyr::select(country, areaEst = unbiased_area, ciArea) %>%
  left_join(popChange, by = 'country') %>%
...
luefChgNaturePlot <- lueChgDFnature %>%
  ggplot(aes(x = reorder(country, lue), y = lue)) +
  geom_hline(yintercept = 0, linetype=2) +
  geom_point(size=3, alpha=0.6, color='#bb63b1') +
  geom_errorbar(aes(ymin=lue-lueCI, ymax=lue+lueCI), width=0, color='#bb63b1') +
  ...
  labs(x = '',
       title = 'b',
       y =expression(atop('Land take natural/semi-natural land', '('~m^-2~Delta~citizen^-1~')')))+
  theme(axis.title.x = element_text(size=9),
        axis.title.y = element_blank())
luefChgNaturePlot


lueChgDFcropland <- areaEstCountry%>%
  filter(takeType == 'separate')%>%
  filter(stratum == 'cropland loss') %>%
  dplyr::select(country, areaEst = unbiased_area, ciArea) %>%
  left_join(popChange, by = 'country') %>%
...
luefChgCroplandPlot <- lueChgDFcropland %>%
  ggplot(aes(x = reorder(country, lue), y = lue))+
  geom_hline(yintercept = 0, linetype=2) +
  geom_point(size=3, alpha=0.6, color='#63c8c9') +
  geom_errorbar(aes(ymin=lue-lueCI, ymax=lue+lueCI), width=0, color='#63c8c9')  +
  ...
  labs(x = '',
       title = 'c',
       y =expression(atop('Land take cropland', '('~m^-2~Delta~citizen^-1~')')))+
  theme(axis.title.x = element_text(size=9),
        axis.title.y = element_blank())
luefChgCroplandPlot


luefChgPlot <- grid.arrange( lueChgLandTakePlot, luefChgNaturePlot, luefChgCroplandPlot, ncol=3,
                          padding = unit(0, "line"),widths=c(1,1,1), newpage = T)

ggsave("./output/luefChgPlot.png", luefChgPlot, width = 24, height=15, units='cm')

#### Land take relative to baseline cover -------------------------------------
# https://clc.gios.gov.pl/doc/clc/CLC_Nomenclature_EN.pdf

baselineClc <- read_csv('./data/From_GEE/clc_baseline_areas.csv') %>%
  filter(type == 'clc') %>%
  mutate(landCover = ifelse(stratum <= 142, 'Artificial',
                          ifelse((stratum >= 211 & stratum <= 223) | (stratum >= 241 & stratum <= 244), 'Cropland',
                                 ifelse(stratum > 421 , 'Water', 'Nature')))) %>%
  group_by(country, landCover, type) %>%
  summarise(area = sum(area)/1000000)
length(unique(baselineClc$country))


baselineClcplus <- read_csv('./data/From_GEE/clc_baseline_areas.csv') %>%
  filter(type == 'clcplus') %>%
  mutate(landCover = ifelse(stratum == 1, 'Artificial',
                            ifelse(stratum == 5, 'Cropland',
                                   ifelse(stratum == 7, 'Water', 'Nature')))) %>%
  group_by(country, landCover, type) %>%
  summarise(area = sum(area)/1000000)

baselineLc <- baselineClc %>%
  bind_rows(baselineClcplus)

EUlandArea <- EUareasEstOut %>%
  filter(takeType == 'merged') %>%
  summarise(areaTot = sum(area_mapped)) %>%
  pull(areaTot)
# Tot baseline artificial perc total land area
baselineLc %>%
  filter(landCover == 'Artificial') %>%
  group_by( type) %>%
  summarise(areaTot = sum(area)/EUlandArea*100)

getEUltStatsRelBaselineArea <- function(){
  
  print(EUareasEstOut %>%
    filter(stratum == 'land take') %>%
    dplyr::select( area, ciArea)  %>%
    cross_join(baselineLc %>%
                 filter(landCover == 'Artificial') %>%
                 group_by( type) %>%
                 summarise(areaTot = sum(area)) ) %>%
    mutate(areaPerc = area / areaTot * 100,
           areaPercCI = ciArea / areaTot * 100))
  ...
  print(EUareasEstOut %>%
    ...
    dplyr::select( area, ciArea)  %>%
    cross_join(baselineLc %>%
                 filter(landCover == 'Nature') %>%
                 group_by( type) %>%
                 summarise(areaTot = sum(area)) ) %>%
    mutate(areaPerc = area / 6 / areaTot * 100,
           areaPercCI = ciArea  / 6 / areaTot * 100) %>%
      mutate(nyrstotheend = 100/areaPerc)) 
...
getEUltStatsRelBaselineArea()

# Comparison to Li et al 2022
# https://ars.els-cdn.com/content/image/1-s2.0-S0169204621002711-mmc1.pdf
# Europe
(27187 + 20631 + 19431 )/ 40

# GLobal
(143000 + 132000  +133000) / 40

ltRelBaseTot <- areaEstCountry %>%
  filter(stratum == 'land take') %>%
  dplyr::select(country, area, ciArea)  %>%
  left_join(baselineLc %>%
              filter(landCover != 'Water') %>%
              group_by(country, type) %>%
              summarise(areaTot = sum(area)), by = 'country' ) %>%
  mutate(areaPerc = area / areaTot * 100,
         areaPercCI = ciArea / areaTot * 100)
ltRelBaseTot

ltRelBaseTotPlot <- ltRelBaseTot %>%
  ggplot(aes(x = reorder(country, areaPerc), y = areaPerc, shape=type)) +
  geom_point(size=2, alpha=0.6, position = position_dodge(width=0.75)) +
  geom_errorbar(aes(ymin=areaPerc-areaPercCI, ymax=areaPerc+areaPercCI), width=0, 
                position = position_dodge(width=0.75)) +
  coord_flip() +
  labs(x = '',
       title = 'a',
       y = 'Land take share of total land area (%)')+
...
ltRelBaseArtPlot <- ltRelBaseArt %>%
  ...
  labs(x = '',
       title = 'b',
       y = 'Land take share of artificial area (%)')+
  theme(legend.position = 'none',
        axis.title.y = element_blank())
ltRelBaseArtPlot

ltRelBaseNat <- areaEstCountry %>%
  filter(stratum == 'nature loss') %>%
  dplyr::select(country, area, ciArea)  %>%
  left_join(baselineLc %>%
              filter(landCover == 'Nature') %>%
...
ltRelBaseNatPlot <- ltRelBaseNat %>%
  mutate(type = ifelse(type == 'clc', 'CLC AL', 'CLC+')) %>%
  ggplot(aes(x = reorder(country, areaPerc), y = areaPerc, shape=type)) +
  geom_point(size=2, alpha=0.6, position = position_dodge(width=0.75), color='#bb63b1') +
  geom_errorbar(aes(ymin=areaPerc-areaPercCI, ymax=areaPerc+areaPercCI), width=0, 
                position = position_dodge(width=0.75), color='#bb63b1') +
  coord_flip() +
  labs(x = '',
       title = 'c',
       y = 'Land take share of natural/semi-natural area',
       shape='Source baseline area')+
...
ltRelBaseCropPlot1 <- ltRelBaseCrop %>%
  filter(country == 'ISL') %>%
  ggplot(aes(x = reorder(country, areaPerc), y = areaPerc, shape=type)) +
  geom_point(size=2, alpha=0.6, position = position_dodge(width=0.75), color='#63c8c9') +
  geom_errorbar(aes(ymin=areaPerc-areaPercCI, ymax=areaPerc+areaPercCI), width=0, 
                position = position_dodge(width=0.75), color='#63c8c9') +
  coord_flip() +
  labs( title = 'd') +
  theme(axis.title = element_blank(),
        legend.position = 'none')
ltRelBaseCropPlot1
ltRelBaseCropPlot2 <- ltRelBaseCrop %>%
  filter(country != 'ISL') %>%
  ggplot(aes(x = reorder(country, areaPerc), y = areaPerc, shape=type)) +
  geom_point(size=2, alpha=0.6, position = position_dodge(width=0.75), color='#63c8c9') +
  geom_errorbar(aes(ymin=areaPerc-areaPercCI, ymax=areaPerc+areaPercCI), width=0, 
  ...
  coord_flip() +
  labs(x = '',
       y = 'Land take share of cropland area (%)') +
  theme(legend.position = 'none',
        axis.title.y = element_blank())
ltRelBaseCropPlot2

ltRelBaseCropPlot <- grid.arrange( ltRelBaseCropPlot1, ltRelBaseCropPlot2, ncol=1,
                          padding = unit(0, "line"),heights=c(1,6), newpage = T)


ltRelPlotOut <- grid.arrange( ltRelBaseTotPlot,ltRelBaseArtPlot, ltRelBaseNatPlot, ltRelBaseCropPlot, ncol=2,
                                   padding = unit(0, "line"),widths=c(1,1), newpage = T)

ggsave("./output/ltRelBaseAreaPlot.png", ltRelPlotOut, width = 20, height=26,units='cm')
...
lueCompPlot1 <- lueDFlandtake %>%
  ...
  theme(axis.title = element_text(size=10),
        ...
        axis.title.y = element_blank())+
  labs(y = expression(atop('Land take total', '('~m^-2~Delta~citizen^-1~')')),
       x = '')
lueCompPlot1
...
lueCompPlot2 <- lueDFnature %>%
  ...
  theme(axis.title = element_text(size=10),
        axis.title.x = element_text(size=9),
        axis.title.y = element_blank())+
  labs(y = expression(atop('Land take natural/semi-natural land', '('~m^-2~Delta~citizen^-1~')')),
       x = '')
lueCompPlot2
...
lueCompPlot3 <- lueDFcropland %>%
  ...
  theme(axis.title = element_text(size=10),
        axis.title.x = element_text(size=9),
        axis.title.y = element_blank())+
  labs(y = expression(atop('Land take cropland', '('~m^-2~Delta~citizen^-1~')')),
       x = '')
...
landuseDriverPlot <- euLUdrivers %>%
  ...
  mutate(stratum = ifelse(stratum == 'nature loss', 'land take natural/semi-natural', 'land take cropland')) %>%
