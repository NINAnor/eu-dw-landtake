 ...
 * 7. Get baseline CLC+ and CLC AL areas
 * 
...
/***
 * 7. Baseline CLC+ and CLC AL areas ------------------------------------------------------
 */


// Areas ----
//grid = grid.filterBounds(ee.Geometry.Rectangle(Map.getBounds()));
var list= grid.reduceColumns(ee.Reducer.toList(), ['CellCode']).get('list').evaluate(function(list){
  print(list)
  
  //list = [18594054]
  
  for (var i = 875; i<875; i++){
    var id = list[i]
    
    var aoi = grid.filter(ee.Filter.eq('CellCode', id)).geometry();
    var countriesSel = countries.filterBounds(aoi)
    
    aoi = aoi.intersection(countriesSel.geometry());
    
    var clcImgGrid = clc18.clip(aoi)
    var clcplusImgGrid = clcplus.clip(aoi)
    
    var proj = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED").filterBounds(aoi).first().select(1).projection()
    
    var stratAreas = countriesSel.map(function(country){
      var cSel = country.get('ISO3_CODE')
      var aoiCountry = aoi.intersection(country.geometry());
      
      var areasClc = getStratAreas(clcImgGrid, aoiCountry,  10, proj);
      var areasClcplus = getStratAreas(clcplusImgGrid, aoiCountry,  10, proj);
      areasClc = areasClc.map(function(ft){
        return ft.set( 'CellCode', id, 'type', 'clc')
      })
      areasClcplus = areasClcplus.map(function(ft){
        return ft.set( 'CellCode', id, 'type', 'clcplus')
      })
      var stratAreas = areasClc.merge(areasClcplus)
      
      stratAreas = stratAreas.map(function(ft){
        return ft.set('country', cSel)
      })
